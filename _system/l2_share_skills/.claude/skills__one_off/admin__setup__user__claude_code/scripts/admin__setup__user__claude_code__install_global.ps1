# admin__setup__user__claude_code__install_global.ps1 - install up-skill as GLOBAL (personal)
...
# this script lives inside the up-skill repo
$repoRoot = (& git rev-parse --show-toplevel).Trim()
if ($LASTEXITCODE -ne 0 -or -not $repoRoot) {
    [Console]::Error.WriteLine('error: this script must run inside the up-skill repo (git rev-parse --show-toplevel failed)')
    exit 1
}
...
Write-Output "installed global up-skill skills into: $homeSkills"
Write-Output "workspace expected at: $HOME\.up-skill__workspace"
Write-Output 'start a new Claude Code session, then say: use up-skill to share / get / list skills'
