# up-skill__sharing__share.ps1 - copy a local skill folder into my skills repo and push it.
# PowerShell equivalent of up-skill__sharing__share.sh.
# usage: up-skill__sharing__share.ps1 <skill-folder|skill-name> [message]
param(
    [string]$Skill = '',
    [string]$Message = ''
)
...
if ([string]::IsNullOrWhiteSpace($Skill)) {
    us_err 'usage: up-skill__sharing__share.ps1 <skill-folder|skill-name> [message]'
    exit 1
}
$src = $Skill
...
if (-not (Test-Path -LiteralPath (Join-Path $src 'SKILL.md'))) {
    us_err "error: not a skill folder (no SKILL.md): $src"
    exit 1
}
