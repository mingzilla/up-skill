# upskill - sharing solution (level 2)
...
| Skills repo | per-member github repo of shared skills: `upskill__skills_repo__<member>` |
| Address book | per-team github repo: `address_book.json` maps member name -> `{folder, repo}` |
| Solution repo | `mingzilla/upskill` - installers + the skills + l2 one-off setups |
| Workspace | hidden, per-machine data folder: `.upskill__workspace` (data only, skills are global) |
...
```text
mingzilla/upskill                       the solution repo
├── README.md
├── upskill__install.sh                 entry installer  (curl | bash, linux/mac/wsl)
├── upskill__install.ps1                entry installer  (irm | iex, windows)
├── upskill__uninstall.sh / .ps1
...
├── upskill__address_book__<team>       e.g. ...__sandbox (public example)
└── upskill__skills_repo__<member>      one per member (leah / ming / myles)

machine runtime (after install)
├── ~/.upskill__workspace               data only
│   ├── upskill/                        solution clone at prod (source anchor)
│   ├── address_books/<team>/            address book + each member skills repo
│   └── upskill__user-config.json       user / team / address_book
...
| Surface | Sees local files | Can run upskill |
|---|---|---|
...
- Invocation is plain language (`upskill`), not a slash command. `/` pickers in some surfaces run entries as commands; skills are not commands.
...
| 2 | walk up from cwd for `upskill__user-config.json` |
| 3 | default `~/.upskill__workspace` (under the user profile) |

## Install / uninstall

| Tool | Command | Effect |
|---|---|---|
| linux/mac/wsl | `curl -fsSL .../upskill__install.sh | bash` | rebuild workspace (delete+recreate) + copy global skills to `~/.claude/skills` |
| windows | `irm .../upskill__install.ps1 | iex` | same, under `%USERPROFILE%` |
| uninstall | `upskill__uninstall.sh / .ps1` (`--yes` / `-Yes`) | remove global skills + workspace; never repos/projects |
...
flowchart LR
    ...
    prod --> use["user uses upskill - skills self-update"]
...
- The user never reruns the installer: on use, the skill pulls the workspace's solution clone
  (`~/.upskill__workspace/upskill`, branch prod) and refreshes the global copies - automatic,
...
| 3 | Member repo name | `upskill__skills_repo__<member>` | frees the `sharing` namespace for skills; reads as a place |
...
| 8 | Workspace discovery | env -> walk-up -> default `~/.upskill__workspace` | a skill always knows where the data is |
